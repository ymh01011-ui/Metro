...
object LastFmBiographyFetcher {
    ...
    suspend fun fetchBiography(artistName: String): String? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val artistObject = JSONObject(json).optJSONObject("artist") ?: return@withContext null
            val bioObject = artistObject.optJSONObject("bio") ?: return@withContext null
            val summary = bioObject.optString("summary")

            cleanBiography(summary).ifEmpty { null }
        } catch (e: Exception) {
            null
        }
    }
